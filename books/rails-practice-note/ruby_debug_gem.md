...
debug gemはターミナルの標準入出力をつかってデバッグコンソールを起動しますが、そのほかにVSCodeから接続してデバッグすることもできます。
...
また、ターミナルからVSCodeを起動するcodeコマンドがないときは、[VSCode CLI](https://code.visualstudio.com/docs/editor/command-line)をインストールしておきます。
...
VSCodeのメニューから 実行 - デバッグの開始 を実行したときにdebug gemをつかう方法です。事前にVSCodeへ[VSCode rdbg Ruby Debugger拡張](https://marketplace.visualstudio.com/items?itemName=KoichiSasada.vscode-rdbg)をインストールしておきます。
